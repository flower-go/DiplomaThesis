...
Reusing the knowledge can lead to lower training times with fewer demands on technical resources (GPU, CPU) and training data size. It even allows to successfully apply automatic processing into domains where labeled data are not available by transferring the knowledge from another domain with enough training examples. In addition, usage of underlying common knowledge between different tasks can also improve the results of learning algorithms on each task. %TODO{as can be seen where?}

One of the first big successes of transfer learning comes from the computer vision field by using models pretrained on ImageNet (currently also on other datasets). ImageNet %TODO cite ImageNet Large Scale Visual Recognition Challenge 
is a big dataset of labeled pictures. %TODO jak vypadají labely 
Many large deep models were trained on this dataset and then applied to different computer vision tasks with great success \citep{Huh}. %TODO nekam ze proc to funguje a co vsechno se uci se stale zkouma
%TODO Ruder obrázek taxonomie transfer learningu
...
\caption{Figure from \protect\citep{Ruder2019} offers possible taxonomy for transfer learning. %TODO co je to domena
...
Contextualization of embeddings is not important only because it solves the problem of homonyms but also because they are supposed to store knowledge independent of any language.  \citep{Feijo2020}.  %TODO proč? : (Liu et al., 2019a; Hewitt and Liang, 2019; Hewitt and Manning, 2019; Tenney et al., 2019a) demonstrate across languages
...
%TODO kromě liu ještě nějaký zdroj na language modelling - bengio2003
...
First attempts were made with autoencoders (read the text, encode and decode back) \citep{Dai2015} and machine translation \citep{Ramachandran2017}, but later papers come with better architectures and various new objectives which are described later in sectio \ref{sec:bert}. %TODO ze to bude lepe popsane pozdeji

%TODO predelat tabulku z Liu2020 aby tam byly i jména atd.

%TODO pridat jak se to pouziva:  nebo jak by mohlo
%\citep{Howard2018} 
...
%TODO proč je to dobré, nějaké výsledky!!!!

The second part of its popularity is the transfer learning feature of BERT. Google %todo autoři článku, 
published pre-trained models for English and later also for Chinese and multilingual model and other monolingual models were also published by other authors. ( %TODO jak bert?)
Training of the language model requires a considerable amount of data 
and computational time, but it is needed to be done just once. When the model is trained, it can be used for many tasks by changing the classification head and training only newly added layers and/or training all layers (but in this case only for a few episodes and with smaller data) and still performing on the state-of-the-art level. %TODO presny popis 
% The performance of multilingual model can be doubt, especially in the case of smaller languages, so more monolingual models were published all over the world after that. This is very important feature. 
% nejake jsou primo v BERTovi

%TODO jak je super, že to je bidirectional narozdíl od...
...
%todo obrazek
BERT uses the concatenation of three types of embeddings as an input representation -- token embeddings, position embeddings, and segment embeddings. %todo figura z berta
...
The BERT model input can be one or two sequences (not necessarily two sentences, but e.g., also paragraphs) \footnote{Different terminology is used here than in the original paper. In \citep{Devlin2019}, \textit{sentence} is a term for a whole part of input (fist or second), while a term \textit{sequence} is used for whole bert input compound from one or two \textit{sentences}}. All words are split into tokens and converted into embeddings with the use of a pretrained embeddings model. One word can be tokenized into more tokens because BERT uses WordPiece %todo zdroj
embeddings \citep{Wu2016}. WordPiece pretrained embedding algorithm was originaly created for the task of Google voice search for Asian languages and is designed to minimize unknown word tokens. %TODO někam napsat proč se tohle děje
...
BERT uses its encoder architecture for each layer, 
 so L encoder layers are finished with one fully connected layer for a specific task. The original paper presents two sizes of architecture: base and large. Their comparison can be seen in figure %TODO dodelat obrazek staci z paperu
 
  

 %TODO obrazek architektury
 ...
 \end{itemize}.
 In the first case, the model is used only once to generate embeddings for all input data. These embeddings are stored and later fed to any other NLP model like regular embeddings. Technically this can be achieved by stacking the second model on the top of BERT and freeze (disable training) on BERT layers, but whit would be time and memory more consuming. This method is valid, the results will more likely be improved by using BERT embeddings over some other types, but as BERT paper suggests, this method loses some language model potential. %TODO jake jsou moznosti vytvorit ty embeddings?
%Semi-supervised Sequence Learning and ULMFiT.
 In the second case, a typical scenario based on  
 is following:
 Classification head or more sophisticated model is stacked on the top of BERT. In the first phase, this whole new model is trained as if it was using only BERT embeddings, i.e., BERT part is frozen. This phase takes a fair amount of episodes %TODO  co je typicke
 and typical learning rate around %TODO doplnit. 
 After that, when the model is not a totally random mess and returns quite decent results, the whole model, including BERT layers, is further trained (finetunned) for few episodes and with a significantly smaller learning rate, e.g. %TODO doplnit.
...
GTP uses just the left context of the word, as a sentence is processed sequentially. ELMo uses LSTM %todo odkaz
