    ...
    def train_epoch(self, dataset, args):
        for batch in dataset.batches(args.batch_size):
            metrics = self.model.train_on_batch(
                batch[dataset.FORMS].word_ids,
                batch[dataset.TAGS].word_ids[..., np.newaxis],
                reset_metrics=True)
    ...
    def evaluate(self, dataset, dataset_name, args):
        for batch in dataset.batches(args.batch_size):
            metrics = self.model.test_on_batch(
                batch[dataset.FORMS].word_ids, batch[dataset.TAGS].word_ids[..., np.newaxis], reset_metrics=False)
