...
class Network:
    def __init__(self, args, labels):
        ...
        bert_output = self.bert(subwords, attention_mask=tf.cast(subwords != 0, tf.float32))[2][-1] #vezmu posledni vrstvu
        # dropout
    ...
    def train(self, omr, args):
        ...
        for e, lr in range(args.epochs):
            for i in range(e):
                network.train_epoch(omr.train, args, lr)
...
#    tf.config.threading.set_intra_op_parallelism_threads(args.threads)
    ...
    facebook = TextClassificationDataset("czech_facebook", tokenizer=tokenizer.tokenize)
